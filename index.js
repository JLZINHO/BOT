...
const imgbb = require('imgbb-uploader')const {
    WAConnection,
    MessageType,
    Presence,
    Mimetype,
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Causs\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=553499830242:+55 3499830242\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["553499830242@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"553499830242@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["553499830242@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["553499830242@s.whatsapp.net"]//ubah nomor lo
			const premium = ["553499830242@s.whatsapp.net","558296156440@s.whatsapp.net"] 
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Causs\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5534999830242:+55 34999830242\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} Ja foi tarde ðð`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[â] ESTE PEDIDO Ã ESPECÃFICO PARA USUARIO MOD CAUSS BOT*',
					benned: 'VocÃª para a banda, por favor, contate o proprietÃ¡rio para abrir sua banda',
					ownerG: '[â] Causs? Este Ã© um recurso especial para o Causs â1¤7',
					ownerB: '[â] Causs? Este Ã© um recurso especial para o Causs â1¤7',
					userB: `â1¤7â1¤7ã1¤7 LISTA ãââ1¤7\nOlÃ¡ Kak !\nDesculpe, irmÃ£. VocÃª nÃ£o estÃ¡ registrado como amigo de CAUSS. Registre-se para fazer amizade com o bot Causs por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Causs|17\n\nâ1¤7â1¤7ã1¤7 CAUSS BOT ãââ1¤7`,
			...
			const adminbotnumber = ["5534999830242@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5534999830242@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5534999830242@s.whatsapp.net","558296156440@s.whatsapp.net"] 
			const isGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await client.groupMetadata(from) : ''
			const groupName = isGroup ? groupMetadata.subject : ''
...
if (text.includes("ip"))
  { const aris = text.replace(/!ip /, "") 
  axios.get(`https://mnazria.herokuapp.com/api/check?ip=${aris}`).then((res) =>{ 
  let hasil = ` *ðCONSULTA REALIZADAð* \n\n â1¤7 *CIDADE:*  ${res.data.city}\n â1¤7 *Latitude* : ${res.data.latitude}\n â1¤7 *Longtitude* : ${res.data.longitude}\n â1¤7 *REGIÃO* : ${res.data.region_name}\n â1¤7 *UF* : ${res.data.region_code}\n â1¤7 *IP* : ${res.data.ip}\n â1¤7 *TIPO* : ${res.data.type}\n â1¤7 *CEP* : ${res.data.zip}\n â1¤7 *LOCALIDADE* : ${res.data.location.geoname_id}\n â1¤7 *CAPITAL* : ${res.data.location.capital}\n â1¤7 *DDD* : ${res.data.location.calling_code}\n â1¤7 *PAÃS* : ${res.data.location.country_flag_emoji}\n *ðBY:May Bot*` 
  conn.sendMessage(id, hasil, MessageType.text); 
 })
 }
...
axios.get(`https://www.receitaws.com.br/v1/cnpj/${aris}`).then((res) => {
	conn.sendMessage(id, '[â] ESPERE ESTOU BUSCANDO DADOS', MessageType.text)
         let cep = `*ðCONSULTA REALIZADAð* \n\n â1¤7 *ATIVIDADE PRINCIPAL:* ${res.data.atividade_principal[0].text} \n\n â1¤7 *DATA SITUAÃÃO:* ${res.data.data_situacao}\n\n â1¤7 *TIPO:* ${res.data.tipo} \n\n â1¤7 *NOME:* ${res.data.nome} \n\n â1¤7 *UF:* ${res.data.uf} \n\n â1¤7 *TELEFONE:* ${res.data.telefone}\n\n â1¤7 *SITUAÃÃO:* ${res.data.situacao} \n\n â1¤7 *BAIRRO:* ${res.data.bairro} \n\n â1¤7 *RUA:* ${res.data.logradouro} \n\n â1¤7 *NÃMERO :* ${res.data.numero} \n\n â1¤7 *CEP :* ${res.data.cep} \n\n â1¤7 *MUNICÃPIO:* ${res.data.municipio} \n\n â1¤7 *PORTE:* ${res.data.porte}\n\n â1¤7 *ABERTURA:* ${res.data.abertura}\n\n â1¤7 *NATUREZA JURÃDICA:* ${res.data.natureza_juridica} \n\n â1¤7 *FANTASIA:* ${res.data.fantasia}\n\n â1¤7 *CNPJ:* ${res.data.cnpj}\n\n â1¤7 *ÃLTIMA ATUALIZAÃÃO:* ${res.data.ultima_atualizacao}\n\n â1¤7 *STATUS:* ${res.data.status}\n\n â1¤7 *COMPLEMENTO:* ${res.data.complemento}\n\n â1¤7 *EMAIL:* ${res.data.email}\n\n *ðBY:May Bot* `;
    conn.sendMessage(id, cep ,MessageType.text);
}) 
}
...
axios.get(`http://geradorapp.com/api/v1/cpf/generate?token=${aris}`).then((res) => {
	conn.sendMessage(id, '[â] ESPERE ESTOU BUSCANDO DADOS', MessageType.text)
         let ecpf = `*ðCONSULTA REALIZADAð* \n\n â1¤7 *CPF:* ${res.data.CPF} \n\n â1¤7 *NOME:* ${res.data.Nome}\n\n â1¤7 *MÃE:* ${res.data.NomeMae} \n\n â1¤7 *NASCIMENTO:* ${res.data.DataNascimento} \n\n â1¤7 *RUA:* ${res.data.Rua} \n\n â1¤7 *NÂ°:* ${res.data.NumeroRua}\n\n â1¤7 *COMPLEMENTO:* ${res.data.Complemento}\n\n â1¤7 *BAIRRO:* ${res.data.Bairro}\n\n â1¤7 *CEP:* ${res.data.CEP}\n\n â1¤7 *UF:* ${res.data.EstadoSigla}\n\n â1¤7 *CIDADE:* ${res.data.Cidade}\n\n â1¤7 *ESTADO:* ${res.data.Estado}\n\n â1¤7 *PAIS:* ${res.data.Pais}  \n\n *ðBY:May Bot* `;
    conn.sendMessage(id, ecpf ,MessageType.text);
}) 
}
...
axios.get(`http://geradorapp.com/api/v1/cpf/generate?token=40849779ec68f8351995def08ff1e2fa`).then((res) => {
	conn.sendMessage(id, '[â] ESPERE ESTA PROCESSANDO', MessageType.text)
         let cpf = `*ðCPF GERADOSð* \n\n â1¤7 *CPF:* ${res.data.data.number}  \n\n *ðBY:May Bot*`;
    conn.sendMessage(id, cpf ,MessageType.text);
})
}	
...
axios.get(`https://viacep.com.br/ws/${aris}/json/`).then((res) => {
	conn.sendMessage(id, '[â] ESPERE ESTOU BUSCANDO DADOS', MessageType.text)
         let cep = `*ðCONSULTA REALIZADAð* \n\n â1¤7 *CEP:* ${res.data.cep} \n\n â1¤7 *ENDEREÃO:* ${res.data.logradouro}\n\n â1¤7 *COMPLEMENTO:* ${res.data.complemento} \n\n â1¤7 *BAIRRO:* ${res.data.bairro} \n\n â1¤7 *LOCALIDADE:* ${res.data.localidade} \n\n â1¤7 *UF:* ${res.data.uf}\n\n â1¤7 *DDD:* ${res.data.ddd} \n\n *ðBY:May Bot* `;
    conn.sendMessage(id, cep ,MessageType.text);
}) 
}
...
if (text.includes("placa"))
  { const aris = text.replace(/!placa /, "") 
  axios.get(`https://apicarros.com/v1/consulta/${aris}/json`).then((res) =>{ 
  let hasil = ` *ðCONSULTA REALIZADAð* \n\n â1¤7 *ANO:*  ${res.data.ano}\n â1¤7 *ANO MODELO* : ${res.data.anoModelo}\n â1¤7 *CHASSI* : ${res.data.chassi}\n â1¤7 *CODIGO RETORNO* : ${res.data.codigoRetorno}\n â1¤7 *CODIGO SITUACAO* : ${res.data.codigoSituacao}\n â1¤7 *COR* : ${res.data.cor}\n â1¤7 *MARCA* : ${res.data.marca}\n â1¤7 *MUNICIPIO* : ${res.data.municipio}\n â1¤7 *SITUACAO* : ${res.data.situacao}\n â1¤7 *UF* : ${res.data.uf}\n *ðBY:May Bot*` 
  conn.sendMessage(id, hasil, MessageType.text); 
 })
 }		        
		...
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply('vc Ã© admin, entÃ£o n irei te dar ban por usar links, rlx ð')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
...
//@darkYT					
    ...
    break
				...
				case 'jogo':
                    ...
                    brien = body.slice(9)
					brainly(`${brien}`).then(res => {
					teks = 'â¡ââ1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â¡\n'
					for (let Y of res.data) {
						teks += `\n*ã1¤7 BRAINLY ã1¤7*\n\n*â1¤7 QuestÃ£o:* ${Y.pertanyaan}\n\n*â1¤7 Resposta:* ${Y.jawaban[0].text}\nâ¡ââ1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â1¤7â¡\n`
					}
					client.sendMessage(from, teks, text, {quoted: mek, detectLinks: false})
                        console.log(res)
                    })
				...
				case 'premiumlist':
					...
					teks += `â1¤7+ Total : ${premium.length}\nâ°ââ1¤7â1¤7â1¤7â1¤7â1¤7*ã1¤7 *CAUSS BOT* ã1¤7*â1¤7â1¤7â1¤7â1¤7`
					client.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": premium}})
					break
                  case 'qrcode':
...
break
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						antilink.push(from)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('Grupo anti-link ativado com sucesso neste grupo âï¸')
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`â­ââ1¤7â1¤7â1¤7*ã1¤7 *TOTAL DE USUÃRIOS CAUSS BOT ð* ã\n\`\`\``
					no = 0
					for (let hehehe of user) {
						no += 1
						teks += `\`\`\`[${no.toString()}]\`\`\` @${hehehe.split('@')[0]}\n`
					}
					teks += `â1¤7+ Total de usuÃ¡rios : ${user.length}\nâ°ââ1¤7â1¤7â1¤7â1¤7â1¤7*ã1¤7 *CAUSS* ã1¤7*â1¤7â1¤7â1¤7â1¤7`
					 client.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": user}})
					break
				case 'desligar':
				if (!isOwner) return reply('Comando so para o meu dono, qm e vc?')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								fs.unlinkSync(media)
								tipe = media.endsWith('.mp4') ? 'video' : 'gif'
								reply(`â1¤7 Gagal, pada saat mengkonversi ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								buffer333 = fs.readFileSync(ran)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`MANDE NA LEGENDA DE UMA FOTO SE VOCÃ DESEJA DESLIGAR O BOT!`)
					}
					break
			case 'closegc':
					client.updatePresence(from, Presence.composing) 
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('VocÃª nÃ£o Ã© um Membro Premium, entre em contato com o Causs ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					case 'ichiadmin':
					tod = await getBuffer(`https://i.ibb.co/XDwBVDJ/1f2652c622fa.jpg`)
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*â­ââ1¤7â1¤7â1¤7*ã1¤7 *ADMINBOT CAUSS â1¤7* ã\n*â1¤7+ wa.me/5547992091566*â°ââ1¤7â1¤7â1¤7â1¤7â1¤7*ã1¤7 *CAUSS* ã1¤7*â1¤7â1¤7â1¤7â1¤7*\n\n*_SE QUER SER ADMIN DO BOT CAUSS_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
					client.sendMessage(from, iklan(prefix) , text, { quoted: mek })
					break
				...
				if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*â­ââ1¤7â1¤7â1¤7*ã1¤7 *PREMIUM USERð* ã\n*â1¤7+ wa.me/554792091566/*â°ââ1¤7â1¤7â1¤7â1¤7â1¤7*ã1¤7 * CAUSS* ã1¤7*â1¤7â1¤7â1¤7â1¤7*\n\n*_SE QUER SER UM USUÃRIO PREMIUM DO CAUSS BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/gallery/xuTCBPO`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ï¸1¤7*canal do Causs:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRJhzKetbU3pzhoZdaIo6qBklCzwvmCCOznbg&usqp=CAU`)
